...
namespace CondominiumAlerts.Features.Features.Condominium.Join
{
    public class JoinCondominiumCommandHandler : ICommandHandler<JoinCondominiumCommand, Result<JoinCondominiumResponce>>
    {
        ...
        public async Task<Result<JoinCondominiumResponce>> Handle(JoinCondominiumCommand request, CancellationToken cancellationToken)
        {
           ...
           Domain.Aggregates.Entities.Condominium condominiumTobeJoined = await _condominiumRepository.GetOneByFilterAsync(c => c.InviteCode == request.CondominiumCode);
            ...
            if (await _condominiumUserRepository.AnyAsync(cu => cu.UserId == request.UserId && cu.CondominiumId == Guid.Parse("89af821d-0228-43bb-ae33-87ea1d503579"), default))
            {
