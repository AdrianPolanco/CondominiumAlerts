...
namespace CondominiumAlerts.Features.Features.Condominium.Join
{
    public class JoinCondominiumValidator : AbstractValidator<JoinCondominiumCommand>
    {
        public JoinCondominiumValidator() { 
            ...
            RuleFor(jc => jc.CondominiumCode).MinimumLength(7).WithMessage("The code is to short, it must be 7 characters long");
            RuleFor(jc => jc.CondominiumCode).MaximumLength(7).WithMessage("The code is to long, it must be 7 characters long");
        }
    }
}
